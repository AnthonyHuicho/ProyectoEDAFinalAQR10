...
public:
    ...
    void initAQR(py::array_t<float, py::array::c_style | py::array::forcecast> data)
    {
        ...
        std::vector<std::vector<float>> dataset(rows, std::vector<float>(cols));
        for (int i = 0; i < rows; i++)
        {
            for (int j = 0; j < cols; j++)
            {
                dataset[i][j] = ptr[i * cols + j];
            }
        }
        if (appr_alg)
        {
            appr_alg->fasebuild(dataset);
        }
    }
...
PYBIND11_PLUGIN(hnswlib)
{
    ...
    py::class_<Index<float>>(m, "Index")
        ...
        .def("initAQR", &Index<float>::initAQR, py::arg("data"))
        .def("init_index",
        ...
        .def("getTiempoAQR",
            [](Index<float> &Index) {
                return Index.appr_alg->getTiempoAQR();
            }
        )
        .def("resetTiempos",
            [](Index<float> &Index) {
                Index.appr_alg->resetTiempos();
            }
        )
