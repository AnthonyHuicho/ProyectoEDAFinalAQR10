...
//                    if (candidate_id == 0) continue;
        ...
        size_t efMultiplier = 2;
        double parametroUmbralGap=0.020;
        double parametroUmbralRatio=1.015;
        int coarsecand=35;
        size_t parametroReranK=12;
        std::vector<float> minjguardado;
        std::vector<float> scalejguardado;
        std::vector<std::vector<uint8_t>> datasetint8;
        std::vector<std::vector<float>> datasetfloat32guardado;
        mutable long long tiempoBase=0;
        mutable long long tiempoAQCompleto=0;
        mutable long long tiempoSoloAQR_=0;
        float sdistguardado=1.0f;
        //fin de parametros auxiliares
        long long getTiempoSoloAQR() const { 
            return tiempoSoloAQR_; 
        }
        long long getTiempoBase() const {
             return tiempoBase; }
        long long getTiempoAQR()  const { 
            return tiempoAQCompleto;  }
        void resetTiempos() {
            tiempoBase = 0;
            tiempoAQCompleto = 0;
            tiempoSoloAQR_ = 0;
        ...
        //Implementacion de codigo para mi proyecto del AQR
        void cuantizarallAQR(const std::vector<float>& original, const std::vector<float>& scale, const std::vector<float>& minj, std::vector<uint8_t>& cuantizado) const
        {
            cuantizado.resize(original.size());
            for (int j = 0; j < (int)original.size(); j++)
            {
                float val = original[j] - minj[j];
                val = val * scale[j];
                if (val < 0.0f){
                    val = 0.0f;
                } 
                ...
                cuantizado[j] = static_cast<uint8_t>(val + 0.5f);
            }
        }
        
        float distcuantizada(const std::vector<uint8_t>& querycuantizada, const std::vector<uint8_t>& xcuantizado) const
        {
            float sumatmp = 0;
            for (int j = 0; j < (int)querycuantizada.size(); j++)
            {
                float dif = (float)querycuantizada[j] - (float)xcuantizado[j];
                sumatmp += dif * dif;
            }
            return (sdistguardado * sumatmp);
        }
        void fasebuild(const std::vector<std::vector<float>>& dataset)
        {
            // STAGE 1 con sampling
            std::vector<float> densidadlocal;
            int knearest = 10;
            float epsilon = 1e-6f;
            densidadlocal.resize(dataset.size());
            int tamdataset = dataset.size();
            ...
            if (tamdataset > 10000){
                 ...
                 tamMuestra = tamdataset;
            }
               

            std::vector<int> indicesMuestra(tamMuestra);
            if (tamMuestra == tamdataset)
            {
            ...
            {
                std::vector<int> todos(tamdataset);
                ...
                for (int i = 0; i < tamMuestra; i++){
                    indicesMuestra[i] = todos[i];
                }
                    
            }

            float densidadPromMuestra = 0.0f;
            std::vector<float> densidadMuestra(tamMuestra);
            int paso = 4;

            for (int i = 0; i < tamMuestra; i++)
            {
                int idxI = indicesMuestra[i];
                std::vector<float> distancias;
                distancias.reserve(tamMuestra - 1);
                ...
                for (int j = 0; j < tamMuestra; j++)
                {
                    ...
                    int idxJ = indicesMuestra[j];
                    float dist = 0.0f;
                    for (int d = 0; d < (int)dataset[idxI].size(); d += paso)
                    {
                        float diff = dataset[idxI][d] - dataset[idxJ][d];
                        dist += diff * diff;
                    }
                ...
                float promedio = 0.0f;
                int kreal = std::min(knearest, (int)distancias.size());
                for (int k = 0; k < kreal; k++){
                    promedio += distancias[k];
                }
                    
                promedio /= kreal;

                densidadMuestra[i] = 1.0f / (promedio + epsilon);
                densidadPromMuestra += densidadMuestra[i];
            }
            densidadPromMuestra /= tamMuestra;

            if (tamMuestra == tamdataset)
            {
                for (int i = 0; i < tamdataset; i++){
                     densidadlocal[i] = densidadMuestra[i];
                }
                   
            }
            else
            {
                std::unordered_map<int, float> densidadMap;
                for (int i = 0; i < tamMuestra; i++){
                    densidadMap[indicesMuestra[i]] = densidadMuestra[i];
                }
                    

                for (int i = 0; i < tamdataset; i++)
                {
                    auto it = densidadMap.find(i);
                    if (it != densidadMap.end()){
                        densidadlocal[i] = it->second;
                    }
                    else{
                        densidadlocal[i] = densidadPromMuestra;
                    }
                        
                }
            }

            float maxdensidad = densidadlocal[0];
            float mindensidad = densidadlocal[0];
            for (int a = 0; a < tamdataset; a++)
            {
                if (densidadlocal[a] > maxdensidad){
                    maxdensidad = densidadlocal[a];
                } 
                if (densidadlocal[a] < mindensidad){
                    mindensidad = densidadlocal[a];
                } 
            }
            float H = (maxdensidad - mindensidad) / (maxdensidad + epsilon);

            // STAGE 2
            std::vector<float> minj(dataset[0].size(), std::numeric_limits<float>::max());
            std::vector<float> maxj(dataset[0].size(), std::numeric_limits<float>::lowest());
            std::vector<float> scalej(dataset[0].size(), 0);
            ...
            for (int i = 0; i < tamdataset; i++){
                ...
                {
                    if (dataset[i][j] < minj[j]) minj[j] = dataset[i][j];
                    if (dataset[i][j] > maxj[j]) maxj[j] = dataset[i][j];
                }
            }
                

            for (int a = 0; a < (int)dataset[0].size(); a++){
                scalej[a] = 255.0f / ((maxj[a] - minj[a]) + epsilon);
            }
                

            std::vector<std::vector<uint8_t>> datasetcuantizado(dataset.size());
            for (int i = 0; i < tamdataset; i++){
                cuantizarallAQR(dataset[i], scalej, minj, datasetcuantizado[i]);
            }
            ...
            {
                float disteu = maxj[j] - minj[j];
                numerador   += disteu * disteu * wj;
                denominador += 255.0f * 255.0f * wj;
            }
            double sdist = std::sqrt(numerador / (denominador + epsilon));

            float delta = 0.0f;

            datasetint8            = datasetcuantizado;
            scalejguardado         = scalej;
            minjguardado           = minj;
            sdistguardado          = sdist;

            // STAGE 3
            int efnuevo = static_cast<int>(ef_construction_ / (1.0f + H * delta));
            efnuevo = std::max(efnuevo, 50);
            ef_construction_ = static_cast<size_t>(efnuevo);

            for (int i = 0; i < tamdataset; i++){
                addPoint(static_cast<const void*>(dataset[i].data()), static_cast<labeltype>(i));
            }
                
        }

        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst>searchBaseLayerAQR(tableint entrypointid, const std::vector<uint8_t>& queryCuantizada, size_t ef) const
        {
            VisitedList *vl = visited_list_pool_->getFreeVisitedList();
            vl_type *visited_array = vl->mass;
            vl_type visited_array_tag = vl->curV;
            ...
            dist_t dist_ep = static_cast<dist_t>(distcuantizada(queryCuantizada, datasetint8[entrypointid]));
            top_candidates.emplace(dist_ep, entrypointid);
            candidate_set.emplace(-dist_ep, entrypointid);
            visited_array[entrypointid] = visited_array_tag;
            dist_t lowerBound = dist_ep;

            while (!candidate_set.empty())
            {
                std::pair<dist_t, tableint> current = candidate_set.top();
                ...
                tableint current_id = current.second;
                ...
                for (size_t j = 1; j <= size; j++)
                {
                    ...
                    dist_t dist = static_cast<dist_t>(distcuantizada(queryCuantizada, datasetint8[candidate_id]));
        ...
        bool earlyTerminationPaper(const std::vector<std::pair<dist_t,tableint>>& candidatos, size_t k) const
        {
        ...
        void exactReranking(std::vector<std::pair<dist_t,tableint>>& candidatos, const std::vector<float>& queryFP32, size_t rerankContador) const {
            for (size_t i = 0; i < rerankContador; i++)
            {
                float distExacta = 0.0f;
                const float* xFP32 = reinterpret_cast<const float*>(
                    getDataByInternalId(candidatos[i].second));
                for (size_t j = 0; j < queryFP32.size(); j++)
                {
                    float diff = queryFP32[j] - xFP32[j];
                    distExacta += diff * diff;
                }
                candidatos[i].first = static_cast<dist_t>(distExacta);
            }
            std::sort(candidatos.begin(), candidatos.begin() + rerankContador,
                [](const auto& a, const auto& b){ return a.first < b.first; });
        }

        std::priority_queue<std::pair<dist_t, labeltype>> AQRimplementacion(const void* query_data, size_t k, std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> top_candidates) const
        {
            std::priority_queue<std::pair<dist_t, labeltype>> result;

            const float* queryFP32 = reinterpret_cast<const float*>(query_data);
            size_t dim = data_size_ / sizeof(float);
            std::vector<float> queryFP32vec(queryFP32, queryFP32 + dim);

            // volcar top_candidates a vector ordenado ascendente
            std::vector<std::pair<dist_t, tableint>> A;
            A.reserve(top_candidates.size());
            while (!top_candidates.empty())
            {
                A.push_back(top_candidates.top());
                top_candidates.pop();
            }
            for (size_t i = 0; i < A.size() / 2; i++)
            {
                std::pair<dist_t,tableint> tmp = A[i];
                A[i] = A[A.size() - 1 - i];
                A[A.size() - 1 - i] = tmp;
            }

            // STAGE 2 — distancia asimetrica
            for (int i = 0; i < (int)A.size(); i++)
            {
                const std::vector<uint8_t>& xhat = datasetint8[A[i].second];
                float distAsym = 0.0f;
                for (int j = 0; j < (int)dim; j++)
                {
                    float xtilde = (float)xhat[j] / scalejguardado[j] + minjguardado[j];
                    float diff   = queryFP32vec[j] - xtilde;
                    distAsym    += diff * diff;
                }
                A[i].first = static_cast<dist_t>(distAsym);
            }

            // ordenar por distancia asimetrica
            std::sort(A.begin(), A.end(), [](const auto& a, const auto& b) {
                return a.first < b.first;
            });

            // STAGE 3
            if (earlyTerminationPaper(A, k) == false)
            {
                size_t rerankContador = std::min((size_t)parametroReranK, A.size());
                exactReranking(A, queryFP32vec, rerankContador);

                std::sort(A.begin(), A.end(),[](const auto& a, const auto& b){ return a.first < b.first; });

            }

            size_t take = std::min(k, A.size());
            for (size_t i = 0; i < take; i++){
                result.push({A[i].first, getExternalLabel(A[i].second)});
            }
        ...
        searchKnn(const void *query_data, size_t k, BaseFilterFunctor *isIdAllowed = nullptr) const{
            if (datasetint8.empty())
            {
                std::cout << "Llama fasebuild antes de knn_query en esta version AQR" << std::endl;

            }
            std::priority_queue<std::pair<dist_t, labeltype>> result;
            if (cur_element_count == 0)
                return result;
            auto t0base = std::chrono::high_resolution_clock::now();
            tableint currObj = enterpoint_node_;
            ...
            if (!datasetint8.empty())
            {
                ...
                size_t dim = data_size_ / sizeof(float);
                std::vector<float> queryFP32vec(queryFP32, queryFP32 + dim);
                std::vector<uint8_t> queryCuantizada;
                cuantizarallAQR(queryFP32vec, scalejguardado, minjguardado, queryCuantizada);

                size_t efAQR;
                if (ef_ > k * efMultiplier){
                    efAQR = ef_;
                }
                    
                else{
                    efAQR = k * efMultiplier;
                }
                auto top_candidates = searchBaseLayerAQR(currObj, queryCuantizada, efAQR);

                auto t1base = std::chrono::high_resolution_clock::now();
                tiempoBase += std::chrono::duration<double>(t1base - t0base).count();

                auto t0aqr = std::chrono::high_resolution_clock::now();
                auto res = AQRimplementacion(query_data, k, top_candidates);
                auto t1aqr = std::chrono::high_resolution_clock::now();
                tiempoAQCompleto += std::chrono::duration_cast<std::chrono::microseconds>(t1aqr - t0base).count();
                tiempoSoloAQR_ += std::chrono::duration_cast<std::chrono::microseconds>(t1aqr - t0aqr).count();
                return res; 
            }

            // sin AQR , es el codigo original pero nomas esta comentado porque uso otro para medir el tiempo
            /*
            std::priority_queue<std::pair<dist_t, tableint>,
                                std::vector<std::pair<dist_t, tableint>>,
                                CompareByFirst> top_candidates;
